...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.condenser.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.condenser.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.condenser.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *shelftemp.ProfileManager, windows *shelftemp.HoldWindowManager) *shelftemp.HoldWindowEvaluator {
	return shelftemp.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *condenser.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *condenser.Controller {
	c := condenser.NewController(bank, maxDelta)

...
import (
	...
	"github.com/lacsar712/lyophdry/internal/condenser"
	"github.com/lacsar712/lyophdry/internal/freezech"
	"github.com/lacsar712/lyophdry/internal/fsm"
...
type App struct {
	...
	plant        *freezech.LyoPlant
	towerFSM     *fsm.TowerFSM
	zones        *freezech.ZoneTable
	...
	condenser      *condenser.Controller
	holdMgr      *shelftemp.HoldWindowManager
	holdEval     *shelftemp.HoldWindowEvaluator
	gradAudit    *shelftemp.OrderedGradientValidator
	guard        *interlock.Guard
	dryFSM       *fsm.LyoFSM
	scheduler    *clock.SegmentScheduler
	avgWindow    *clock.SublimWindow
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := freezech.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := freezech.NewLyoPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(shelftemp.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := condenser.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := shelftemp.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    shelftemp.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.condenser = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	if pc, ok := a.clk.(*clock.ProcessClock); ok {
		...
		a.avgWindow = clock.NewSublimWindow(a.clk, time.Duration(cfg.SublimWindowMinutes)*time.Minute)
	}
	a.dryFSM = fsm.NewLyoFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewLyoRamp(a.clk, cfg.ProcessTick(), cfg.LyoRampSteps)
